...
@HOOKS.register_module()
class TgnetEvaluatorFPS(HookBase):
    def after_epoch(self):
        if self.trainer.cfg.evaluate:
            self.eval()

    def eval(self):
        self.trainer.logger.info(">>>>>>>>>>>>>>>> Start Tgnet FPS Evaluation >>>>>>>>>>>>>>>>")
        self.trainer.model.eval()
        for i, input_dict in enumerate(self.trainer.val_loader):
            for key in input_dict.keys():
                if isinstance(input_dict[key], torch.Tensor):
            ...
            with torch.no_grad():
                output_dict = self.trainer.model(input_dict)
            output_seg = output_dict["seg_logits"]
            output_offset = output_dict['offset_logits']
        ...
        all_acc = sum(intersection) / (sum(target) + 1e-10)
        self.trainer.logger.info(
            "Val result: mIoU/mAcc/allAcc/offset/dir {:.4f}/{:.4f}/{:.4f}/{:.4f}/{:.4f}.".format(
                m_iou, m_acc, all_acc, loss_offset, loss_dir
            )
        )
        for i in range(self.trainer.cfg.data.num_classes):
            self.trainer.logger.info(
