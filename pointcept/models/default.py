...
class TgnetSegmentor(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        # 偏移并聚类
        # todo 通过loss查看cls seg的结果，这个不做优化也无法优化
        ...
        # loss_mask, mask_logits, mask_segment_logits = self.split_tooth_in_one_batch(point, clustering, cls_to_label, offset_logits, cls_seg_logits, cls_segment_logits)
        
        # mask_logits = self.mask_head(mask_feat)
        ...
        # eval
            ...
            # print('train loss mask', loss_mask)
    ...
    # def get_single_tooth_point(self, point, phase):
    #     if phase == 'train':
            
        
    # def get_loss_mask(self, point, clustering, cls_to_label, offset_logits, cls_seg_logits, cls_segment_logits):
    #     self.logger.info(f"开始制作第二模块训练的输入")
    #     self.get_single_tooth_point()
